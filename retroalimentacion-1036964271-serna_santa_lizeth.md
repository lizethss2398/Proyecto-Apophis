# Retroalimentación de proyecto computacional

- Nombre completo: SERNA SANTA LIZETH
- Cédula: 1036964271

## Archivos revisados

- Apophis_Proy_LSS_v_f.ipynb (versión final entregada)

## Historial de commits

- Repositorio original: https://github.com/lizethss2398/Proyecto-Apophis
- Fecha de creación del repositorio: 2026-03-19 (antes del 14 de abril: si)
- Total de commits desde creación del repo: 5
- Primer commit: 2026-03-19
- Último commit: 2026-05-25
- Días activos con commits: 4
- Semanas activas con commits: 4
- Mayor pausa entre commits: 38 días

### Aplicación al repositorio

- Inicio oportuno: 5.0
- Constancia semanal: 3.0
- Regularidad: 2.0
- Iteración: 3.0
- Nota de ritmo de commits (promedio): 3.25 / 5.0

Interpretación breve: Ritmo intermedio con concentración del trabajo en pocas semanas; se observó una pausa larga de 38 días.

## Retroalimentación

- Archivo revisado: Apophis_Proy_LSS_v_f.ipynb (versión final entregada)

### Aspectos positivos

- Se evidencia el objetivo central del proyecto (trayectoria de Apophis, contexto de 2029 y estimación de acercamiento a la Tierra).
- Se aplican varias estrategias de modelado/validación y no un único experimento.
- El tamaño de las celdas de código es razonable para lectura y mantenimiento.
- Muy buena la elección de comenzar los experimentos numéricos por el problema de los dos cuerpos, pero en ese mismo orden debería seguir el experimento con el CRTBP.

### Aspectos por mejorar

- En el experimento con el problema de los dos cuerpos falto calcular la distancia mínima a la Tierra de Apophis. [Mejorado]
- Falta fortalecer la parte de análisis y reflexión; no basta con mostrar resultados sin interpretarlos. [Mejorado]
- La narrativa del notebook debe mejorar: muchas celdas de código no están antecedidas por explicación. [Mejorado]
- Se recomienda cerrar con una sección unica de conclusiones (3-5 puntos) que sintetice hallazgos físicos y computacionales y responda directamente al objetivo del proyecto. [Mejorado]

### Valoración global

- El trabajo tiene avances técnicos, pero necesita mejoras de fondo en comunicación de resultados y/o cumplimiento explícito del objetivo.
- Estado de recepción: requiere cambios antes de recepción final.

## Valoración final

- Mejoras implementadas: 5.00 (70%)
- Aplicación al repositorio: 3.25 (10%)
- Aspectos positivos: 3.75 (10%)
- Valoración global: 3.50 (10%)

**Nota final (redondeada)**: 0.7×5.00 + 0.1×3.25 + 0.1×3.75 + 0.1×3.50 = (redondeado) 4.6
